...
- **Debriefs**: voice- and text-first, and NOT gated behind a fixture — a player can open a debrief and talk to Echo at any time, about anything on his mind, not only after a match. Fixture-triggered is the ideal, common case, not a requirement. Framed as "debrief with Echo," never "journaling," never "coaching session." Between matches: shorter check-ins (training, recovery) work exactly the same way.
...
- **Memory that scales**: every debrief gets a compact AI-generated summary (a couple of sentences plus a handful of short tagged "signals" — e.g. *"hesitated before shooting," "blamed teammates for the goal"*) regenerated after every turn. This, not raw transcripts, is what Echo reads as history — summaries are cheap enough to cover a real season's worth of debriefs instead of the last handful, which is what makes cross-debrief pattern recognition genuine rather than a hopeful guess over a thin window. Pattern recognition is explicitly instructed to hunt for recurring negative/self-limiting signals (hesitation, blame-shifting, fear, confidence collapse after a mistake) and name them plainly, not just whatever happens to come up.
...
- **Mind map** (node/link, Obsidian-style) — resolved, no longer an open fork:
  - **Nodes can come from the player OR from Echo's own detected patterns (revised 2026-09-17 — originally "never Echo's").** The original rule existed to keep the map honest — a single AI guess dressed up as a discovery would be worse than no map at all. That risk is now handled a different way: Echo is only allowed to offer a pattern-based node when the same signal has genuinely recurred across multiple separate debriefs (never from a single occurrence, never inflated), so the bar for an Echo-originated node is real repetition, not a hunch. Player-authored nodes (the player linking two things themselves — a decision and an outcome, a trigger and a response) remain the other, still-valid path.
  - Echo never auto-saves, regardless of origin. It offers, lightly, at a natural close in the thread — never interrupting to announce a feature. The offer is three short lines: the mechanism in plain football language, one line of felt-not-cited research grounding ("there's real grounding to this..."), then the node label.
  - If declined, drop it instantly, no second offer that entry. For player-authored nodes, reserve the offer for moments the player is genuinely reasoning ("I think," "I felt like") — not reporting. Firing every entry turns it into noise and trains the player to expect a prompt, which cheapens it.
